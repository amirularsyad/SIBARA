...
$query_ho = $koneksi->query("SELECT nama, posisi, departemen, jabatan
                             FROM data_karyawan
                             WHERE departemen = 'MIS' AND jabatan = 'Staf' AND dihapus = 0
                             ORDER BY nama ASC");
if ($query_ho) {
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary overflow-x-hidden">
    ...
    <script>
        ...
        function populateActorSelects(mode, presetPembuat, presetPemeriksa) {
        ...
        function removeExistingImage(button, id) {
            ...
            var hidden = document.getElementById('hapus_gambar_' + id);
            if (hidden) {
                hidden.value = 'hapus';
            }
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('#input-pembuat').on('change', function() {
                populateActorSelects('input', $(this).val(), $('#input-pemeriksa').val());
            });
            ...
            function renderEditForm(data, barangList, gambarList) {
                ...
                for (var i = 0; i < gambarList.length; i++) {
                    ...
                    gambarHTML += '' +
                        ...
                        '   <input type="hidden" name="gambar_lama_id[]" value="' + escapeHtml(g.id) + '">' +
                        '   <input type="hidden" name="hapus_gambar[]" id="hapus_gambar_' + escapeHtml(g.id) + '" value="">' +
                        '   <input type="file" name="gambar_lama_file[' + escapeHtml(g.id) + ']" accept="image/*" class="form-control mb-2" onchange="previewReplaceImage(this)">' +
                        '   <input type="text" name="keterangan_gambar_lama[' + escapeHtml(g.id) + ']" class="form-control mb-2" placeholder="Keterangan gambar (opsional)" value="' + escapeHtml(g.keterangan ? g.keterangan : '') + '">' +
                        '   <img src="' + escapeHtml(g.file_path) + '" style="display:block;">' +
                        '   <button type="button" class="btn btn-danger btn-sm mt-2" onclick="removeExistingImage(this, ' + escapeHtml(g.id) + ')"><i class="bi bi-trash3-fill"></i></button>' +
                        '</div>';
                }
                ...
                body.innerHTML = '' +
                    ...
                    '   <input type="hidden" name="jabatan_pembuat" id="jabatan_pembuat_edit" value="' + escapeHtml(data.jabatan_pembuat ? data.jabatan_pembuat : '') + '">' +
                    '   <input type="hidden" name="jabatan_pemeriksa" id="jabatan_pemeriksa_edit" value="' + escapeHtml(data.jabatan_pemeriksa ? data.jabatan_pemeriksa : '') + '">' +
                ...
                populateActorSelects('edit', data.pembuat ? data.pembuat : '', data.pemeriksa ? data.pemeriksa : '');

                $('#edit-pt').on('change', function() {
                    setIdPtBySelect('edit-pt', 'id_pt_edit');
                    populateActorSelects('edit');
                    if ($.fn.DataTable.isDataTable('#myTable2')) {
                        $('#myTable2').DataTable().draw();
                    }
                });
                $('#edit-pembuat').on('change', function() {
                    populateActorSelects('edit', $(this).val(), $('#edit-pemeriksa').val());
                });
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function buildApprovalTable(data) {
                var isHO = String(data.pt || '') === 'PT.MSAL (HO)';

                var jabatanPembuat = '-';
                var jabatanPemeriksa = '-';

                if (isHO) {
                    jabatanPembuat = 'Staf MIS';
                    jabatanPemeriksa = 'Staf MIS';
                } else {
                    jabatanPembuat = data.jabatan_pembuat ? data.jabatan_pembuat : '-';
                    jabatanPemeriksa = data.jabatan_pemeriksa ? data.jabatan_pemeriksa : '-';
                }
            ...
            $(document).on('click', '.btn-detail-ba-pemutihan', function(e) {
                ...
                fetch('get_detail.php?id=' + encodeURIComponent(id), {
                    ...
                    .then(function(res) {
                        ...
                        var html = '<h2>Detail Data Pemutihan ' + escapeHtml(data.nomor_ba ? data.nomor_ba : '') + '/PPA/MIS/' + escapeHtml(romawi) + '/' + escapeHtml(tahun) + '</h2>';
