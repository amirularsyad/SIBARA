...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary overflow-x-hidden">
                                ...
                                <div class="custom-input-form col-8">
                                    ...
                                    <div class="row mt-3 border border-1 p-2 rounded-2 me-1">
                                        ...
                                        <div class="row d-none" id="site-extra-row-input">
                                            ...
                                            <div class="col-md-6 mb-3">
                                                <div class="input-group">
                                                    <span class="input-group-text">Jabatan Pengguna</span>
                                                    <input type="text"
                                                        name="jabatan_dept_pengguna"
                                                        id="input-jabatan-dept-pengguna-site"
                                                        class="form-control"
                                                        value=""
    ...
    <script>
        ...
        function getFormContext(mode) {
            ...
            if (mode === 'edit') {
                return {
                    ...
                    jabatanDeptPenggunaHidden: $('#jabatan_dept_pengguna_edit'),
                    deptRow: $('#dept-row-edit'),
                    siteDeptPenggunaInput: $('#edit-dept-pengguna-site'),
                    siteJabatanDeptPenggunaInput: $('#edit-jabatan-dept-pengguna-site'),
                    siteExtraRow: $('#site-extra-row-edit')
                };
            }
            return {
                ...
                jabatanDeptPenggunaHidden: $('#jabatan_dept_pengguna_input'),
                deptRow: $('#dept-row-input'),
                siteDeptPenggunaInput: $('#input-dept-pengguna-site'),
                siteJabatanDeptPenggunaInput: $('#input-jabatan-dept-pengguna-site'),
                siteExtraRow: $('#site-extra-row-input')
            };
        }
        ...
        function syncSiteExtraFields(mode, presetDeptPengguna, presetJabatanDeptPengguna) {
            var ctx = getFormContext(mode);
            var selectedPT = ctx.pt;
            var isHO = isHoPT(selectedPT);
            ...
            if (!ctx.siteDeptPenggunaInput.length || !ctx.siteJabatanDeptPenggunaInput.length) {
                updateDeptPengguna(mode);
                return;
            }

            ctx.siteDeptPenggunaInput.prop('required', !isHO);
            ctx.siteJabatanDeptPenggunaInput.prop('required', !isHO);

            if (isHO) {
                ctx.siteDeptPenggunaInput.val('');
                ctx.siteJabatanDeptPenggunaInput.val('');
                updateDeptPengguna(mode);
                return;
            }

            var currentDeptPengguna = (typeof presetDeptPengguna !== 'undefined' && presetDeptPengguna !== null) ?
                presetDeptPengguna :
                ctx.siteDeptPenggunaInput.val();

            var currentJabatanDeptPengguna = (typeof presetJabatanDeptPengguna !== 'undefined' && presetJabatanDeptPengguna !== null) ?
                presetJabatanDeptPengguna :
                ctx.siteJabatanDeptPenggunaInput.val();

            ctx.siteDeptPenggunaInput.val(currentDeptPengguna ? currentDeptPengguna : '');
            ctx.siteJabatanDeptPenggunaInput.val(currentJabatanDeptPengguna ? currentJabatanDeptPengguna : '');

            updateDeptPengguna(mode);
        }

        function populateActorSelects(mode, presetDepartemen, presetDeptPengguna, presetJabatanDeptPengguna) {
            var ctx = getFormContext(mode);
            var selectedPT = ctx.pt;
            ...
            if (!selectedPT) {
                ...
                if (ctx.siteDeptPenggunaInput && ctx.siteDeptPenggunaInput.length) {
                    ctx.siteDeptPenggunaInput.prop('required', false).val('');
                }
                if (ctx.siteJabatanDeptPenggunaInput && ctx.siteJabatanDeptPenggunaInput.length) {
                    ctx.siteJabatanDeptPenggunaInput.prop('required', false).val('');
                }
                return;
            }
            ...
            syncSiteExtraFields(mode, presetDeptPengguna, presetJabatanDeptPengguna);
        }
        ...
        function removeExistingImage(btn, index) {
            var wrap = btn.closest('.custom-image-item');
            var hidden = document.getElementById('hapus_gambar_' + index);
        ...
        function previewReplaceImage(input) {
            ...
            var wrapper = input.closest('.custom-image-item');
            if (!wrapper) {
                return;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderEditForm(data, barangList, gambarList) {
                ...
                var editDeptPenggunaValue = isHOData ? '' : (data.dept_pengguna ? data.dept_pengguna : '');
                var editJabatanDeptPenggunaValue = isHOData ? '' : (data.jabatan_dept_pengguna ? data.jabatan_dept_pengguna : '');
                ...
                if (gambarList && gambarList.length > 0) {
                    for (var i = 0; i < gambarList.length; i++) {
                        var g = gambarList[i] || {};
                        var filePathRaw = g.file_path ? g.file_path : '';
                        var filePathView = normalizeImagePath(filePathRaw);

                        gambarHTML += '' +
                            '<div class="custom-image-item existing-image-item" data-row-index="' + i + '">' +
                            '   <div class="d-flex justify-content-between align-items-start gap-2">' +
                            '   </div>' +

                            '   <input type="hidden" name="gambar_lama_id[]" value="' + escapeHtml(g.id ? g.id : 0) + '">' +
                            '   <input type="hidden" name="gambar_lama_path[]" value="' + escapeHtml(filePathRaw) + '">' +
                            '   <input type="hidden" name="hapus_gambar[' + i + ']" id="hapus_gambar_' + i + '" value="">' +

                            '   <img src="' + escapeHtml(filePathView) + '" class="w-100 mb-2" style="display:block; cursor:pointer;" onclick="window.open(this.src, \'_blank\')">' +

                            '   <input type="file" name="gambar_lama_file[' + i + ']" accept="image/*" class="form-control mb-2" onchange="previewReplaceImage(this); markEditImageChanged();">' +

                            '   <input type="text" name="keterangan_gambar_lama[' + i + ']" class="form-control mb-2" placeholder="Keterangan gambar (opsional)" value="' + escapeHtml(g.keterangan ? g.keterangan : '') + '" oninput="markEditImageChanged()">' +
                            '       <button type="button" class="btn btn-danger btn-sm" onclick="removeExistingImage(this, ' + i + ')" title="Hapus gambar">' +
                    ...
                    //+
                        ...
                        // '   Belum ada gambar lama.' +
                        // '</div>';
                }

                function syncEditPenggunaFieldsBeforeSubmit() {
                    var pt = $('#edit-pt').val();
                    ...
                    $('#dept_pengguna_edit').val($('#edit-dept-pengguna-site').val());
                    $('#jabatan_dept_pengguna_edit').val($('#edit-jabatan-dept-pengguna-site').val());
                }
                ...
                body.innerHTML = '' +
                    ...
                    '                           <div class="input-group">' +
                    '                               <span class="input-group-text">Pengguna</span>' +
                    '                               <input type="text" name="dept_pengguna_view" id="edit-dept-pengguna-site" class="form-control" value="' + escapeHtml(editDeptPenggunaValue) + '" placeholder="Masukkan pengguna" maxlength="100" required>' +
                    '                           </div>' +
                    '                       </div>' +
                    '                       <div class="col-md-6 mb-3">' +
                    '                           <div class="input-group">' +
                    '                               <span class="input-group-text">Jabatan Pengguna</span>' +
                    '                               <input type="text" name="jabatan_dept_pengguna_view" id="edit-jabatan-dept-pengguna-site" class="form-control" value="' + escapeHtml(editJabatanDeptPenggunaValue) + '" placeholder="Masukkan jabatan pengguna" maxlength="100" required>' +
                    '                           </div>' +
                    '                       </div>' +
                    '                    </div>' +
                '               </div>' +
                ...
                '                       <div id="edit-gambar-container">' + gambarHTML + '</div>' +
                    ...
                    '                       <button type="button" class="btn btn-primary w-75 align-self-center mb-1" onclick="tambahGambar(\'edit-gambar-container\', \'gambar_baru[]\', \'keterangan_gambar_baru[]\'); markEditImageChanged();">+ Tambah Gambar</button>' +
                    '                   </div>' +
                    '               </div>' +
                    '               <div class="mt-3">' +
                    '                   <div class="input-group">' +
                ...
                $('#popupEditBody').find('form').off('submit.syncPengguna').on('submit.syncPengguna', function() {
                    syncEditPenggunaFieldsBeforeSubmit();
                });
                ...
                populateActorSelects(
                    'edit',
                    editDepartemenValue,
                    editDeptPenggunaValue,
                    editJabatanDeptPenggunaValue
                );
                ...
                $('#edit-pt').on('change', function() {
                    ...
                    populateActorSelects(
                        'edit',
                        $('#edit-departemen').val(),
                        $('#edit-dept-pengguna-site').val(),
                        $('#edit-jabatan-dept-pengguna-site').val()
                    );
                    if ($.fn.DataTable.isDataTable('#myTable2')) {
                        $('#myTable2').DataTable().draw();
                    }
    ...
    <?php
    // ======================
    // DEBUG DATA BARANG KE CONSOLE
    // ======================
    $debug_assets_report = array(
        'session_pt_raw'      => isset($_SESSION['pt']) ? $_SESSION['pt'] : null,
        'pt_list'             => $pt_list,
        'pt_map'              => $pt_map,
        'id_pt_list'          => $id_pt_list,
        'id_pt_sql'           => $id_pt_sql,
        'query_assets2'       => $query_assets2,
        'koneksi2_connect_errno' => isset($koneksi2->connect_errno) ? $koneksi2->connect_errno : null,
        'koneksi2_connect_error' => isset($koneksi2->connect_error) ? $koneksi2->connect_error : null,
        'query_status'        => ($result_assets2 ? 'BERHASIL' : 'GAGAL'),
        'query_error'         => (!$result_assets2 && isset($koneksi2->error)) ? $koneksi2->error : null,
        'num_rows'            => ($result_assets2 instanceof mysqli_result) ? $result_assets2->num_rows : 0,
        'sample_rows'         => array()
    );

    if ($result_assets2 instanceof mysqli_result) {
        $debug_sample_rows = array();
        $debug_count = 0;

        while (($debug_row = $result_assets2->fetch_assoc()) && $debug_count < 5) {
            $debug_sample_rows[] = $debug_row;
            $debug_count++;
        }

        $debug_assets_report['sample_rows'] = $debug_sample_rows;

        // kembalikan pointer hasil query supaya loop/render asli tetap bisa jalan
        $result_assets2->data_seek(0);
    }
    ?>
    <script>
        console.group('DEBUG DATA BARANG - ba_pemutihan.php');
        console.log('REPORT:', <?php echo json_encode($debug_assets_report, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>);
        console.groupEnd();
    </script>

    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var rows = document.querySelectorAll('tr.barang-row');

            console.group('DEBUG RENDER BARANG DI DOM');
            console.log('Jumlah tr.barang-row:', rows.length);

            if (rows.length > 0) {
                console.log('Sample baris pertama:', {
                    idpt: rows[0].dataset.idpt || null,
                    pt: rows[0].dataset.pt || null,
                    po: rows[0].dataset.po || null,
                    coa: rows[0].dataset.coa || null,
                    kode: rows[0].dataset.kode || null,
                    merk: rows[0].dataset.merk || null
                });
            } else {
                console.warn('Tidak ada baris barang yang ter-render di DOM.');
            }
