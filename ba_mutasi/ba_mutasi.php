...
$ptPertama = !empty($ptUserList) ? $ptUserList[0] : '';

$pt_user   = $ptUserList;

$daftar_pt = $ptUserList;
...
<body  class="layout-fixed sidebar-expand-lg bg-body-tertiary overflow-x-hidden">
    ...
    $filter_pt = isset($_GET['pt']) ? trim($_GET['pt']) : '';
    if ($filter_pt === '') {
        $filter_pt = $ptPertama;
    }

    // validasi: kalau user coba PT yang bukan miliknya, paksa balik default
    if ($filter_pt !== 'all' && !in_array($filter_pt, $ptUserList, true)) {
        $filter_pt = $ptPertama;
    }
    ...
    //     $where_clauses[] = "(pt_asal = ? OR pt_tujuan = ?)";
    //     $params[] = $filter_pt;
    //     $params[] = $filter_pt;
    //     $types .= 'ss';
    // }
    ...
    if (!empty($filter_pt) && $filter_pt !== 'all') {
        ...
        // mode "all" = semua PT milik user (bukan semua PT perusahaan)
        if (!empty($ptUserList)) {
            $placeholders = implode(',', array_fill(0, count($ptUserList), '?'));

            $where_clauses[] = "(pt_asal IN ($placeholders) OR pt_tujuan IN ($placeholders))";

            // bind 2x (asal & tujuan)
            foreach ($ptUserList as $ptName) { $params[] = $ptName; $types .= 's'; }
            foreach ($ptUserList as $ptName) { $params[] = $ptName; $types .= 's'; }
        } else {
            // jika user tidak punya PT, jangan tampilkan data
            $where_clauses[] = "1=0";
        }
    }
    ...
    // $id_pt_list = array_values(array_unique($id_pt_list));

    // if (!empty($id_pt_list)) {
    //     $where_barang = " WHERE a.id_pt IN (" . implode(',', $id_pt_list) . ") ";
    // } else {
    //     $where_barang = " WHERE 1=0 ";
    // }
    ...
    // if (!$result_assets) {
    // die('Query assets gagal: ' . $koneksi2->error);
    // }
    ...
    <section class="table-wrapper bg-white position-relative overflow-visible d-flex flex-column">
        <h2>Daftar Berita Acara Mutasi Asset Internal</h2>
        ...
        <form method="GET" class="mb-3 d-flex flex-wrap gap-3">
            ...
            <select name="pt" class="form-select" onchange="this.form.submit()" style="width: 200px;">
                ...
                <?php
                $isHOUser = in_array('PT.MSAL (HO)', $ptUserList, true);
                if ($isHOUser): ?>
                    <option value="all" <?= ($filter_pt === 'all') ? 'selected' : '' ?>>Semua PT</option>
                <?php endif; ?>

                <?php foreach ($ptUserList as $ptOpt): ?>
                    <option value="<?= htmlspecialchars($ptOpt); ?>" <?= ($filter_pt === $ptOpt) ? 'selected' : '' ?>>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            $approvalFields = array(
                                ...
                                isset($row['approval_6']) ? $row['approval_6'] : 0,
                                isset($row['approval_7']) ? $row['approval_7'] : 0,
                                isset($row['approval_8']) ? $row['approval_8'] : 0,
                                isset($row['approval_9']) ? $row['approval_9'] : 0,
                                isset($row['approval_10']) ? $row['approval_10'] : 0,
                                isset($row['approval_11']) ? $row['approval_11'] : 0
                            );
                            ...
                            <?php if($row['pending_hapus'] === 1 && $_SESSION['nama'] === $row['pembuat']){ ?>
                                <br>
                    ...
                    <div class="row position-relative">
                        ...
                        <div class="custom-input-form col-8">
                            ...
                            <div class="col-4">
                                ...
                                <input type="text" inputmode="numeric" pattern="[0-9]{1,3}" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,3);" onblur="if(this.value!==''){ this.value=this.value.padStart(3,'0');}" class="form-control" maxlength="3" name="nomor_ba" id="nomor_ba" value="<?= $nomor_ba_baru ?>" required>
                                </div>
                            </div>
                            </div>
                        ...
                        <div class="custom-input-form col-8">
                            ...
                            <div class="row">
                                ...
                                <div class="col-4">
                                    <div class="input-group" style="width:180px;">
                                        <span class="input-group-text">Nomor BA</span>
                                        <input type="text" class="form-control" name="nomor_ba" id="nomor_ba_edit" maxlength="3" inputmode="numeric" pattern="[0-9]{1,3}" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,3);" onblur="if(this.value!==''){ this.value=this.value.padStart(3,'0');}" required>
                                    </div>
                                </div>
                            </div>
        ...
        <div id="popupBoxDataBarang" class="popup-box position-absolute bg-white top-0 rounded-1 flex-column justify-content-start align-items-center p-2" style="height: max-content;align-self: center;z-index: 10;width: 95%;">
            ...
            <div class="w-100" style="height: max-content;">
                <table id="myTable2" class="table table-bordered table-striped text-center" style="width: 100%;">
                    ...
                    <tbody>
                        <!-- diisi via AJAX berdasarkan lokasi_asal -->
                    </tbody>
...
<script> // ===== Data Barang (INPUT) =====
    document.addEventListener('DOMContentLoaded', function () {

        // 1) DataTable sumber barang (popup) - simpan data sebagai OBJECT
        const table2 = $('#myTable2').DataTable({
            responsive: true,
            autoWidth: true,
            scrollY: '200px',
            ...
            columns: [
                ...
                { data: 'pt', defaultContent: '-' },
                { data: 'no_po', defaultContent: '-' },
                { data: 'serial_number', defaultContent: '-' },
                { data: 'jenis_perangkat', defaultContent: '-' },
                { data: 'kode_assets', defaultContent: '-' },
        ...
        // 2) Table3 (list terpilih di popup) - biarkan seperti punyamu
        window.table3 = $('#myTable3').DataTable({
            responsive: true,
            autoWidth: true,
            scrollY: '200px',
        ...
        // 3) Tabel barang terpilih di form input - biarkan seperti punyamu
        window.tableBarangMutasi = $('#tabelBarangMutasi').DataTable({
            responsive: true,
            autoWidth: true,
            scrollY: '200px',
        ...
        // selected items global (untuk cegah duplikat)
        window.selectedItems = window.selectedItems || [];

        // 4) Anti-race request (FIX BUG #2: sering kosong karena request ketimpa)
        let lastReq = 0;

        async function renderAssetsByPT(pt) {
            const reqId = ++lastReq;
            ...
            // jangan clear dulu sebelum data valid didapat
            try {
                table2.processing(true);

                const res = await fetch('get_assets_by_pt.php?pt=' + encodeURIComponent(pt), { cache: 'no-store' });
                if (reqId !== lastReq) return; // ada request baru, abaikan hasil lama
                ...
                // isi table2 sekaligus (lebih cepat & stabil)
                table2.clear();
                table2.rows.add(items);
                table2.draw(false);

                // kalau popup baru saja muncul, kadang perlu adjust kolom
                setTimeout(function () {
                    table2.columns.adjust();
                    if (table2.responsive) table2.responsive.recalc();
                }, 50);

            } catch (err) {
                if (reqId !== lastReq) return;
                console.error('❌ load asset gagal:', err);
                table2.clear().draw(false);
            } finally {
                try { table2.processing(false); } catch (e) {}
            }
        }

        // 5) Saat lokasi asal berubah -> reset list terpilih & reload barang
        const lokasiAsalEl = document.getElementById('lokasi_asal');
        if (lokasiAsalEl) {
            lokasiAsalEl.addEventListener('change', function () {
                window.table3.clear().draw(false);
                window.tableBarangMutasi.clear().draw(false);
                window.selectedItems = [];
                renderAssetsByPT(this.value);
            });
        }

        // 6) Saat popup dibuka -> load sesuai lokasi asal saat ini
        document.addEventListener('click', function (e) {
            if (e.target.closest('.tombolDataBarangPopup')) {
                const pt = (document.getElementById('lokasi_asal') || {}).value || '';
                renderAssetsByPT(pt);

                // adjust setelah popup tampil
                setTimeout(function () {
                    table2.columns.adjust();
                    if (table2.responsive) table2.responsive.recalc();
                }, 80);
        ...
        // 7) Klik baris di myTable2 => masuk myTable3 (pakai DATA dari DataTables, bukan dataset)
        $('#myTable2 tbody').on('click', 'tr', function () {
            let tr = $(this);

            // kalau klik baris "child" responsive, ambil parent-nya
            if (tr.hasClass('child')) tr = tr.prev();

            const item = table2.row(tr).data();
            if (!item) return;

            const id = item.id_assets;
            if (!id) return;

            if (window.selectedItems.indexOf(id) !== -1) {
                alert('Barang ini sudah ditambahkan!');
                return;
            }
        ...
        // 8) Hapus item dari myTable3
        $('#myTable3 tbody').on('click', '.delete-row', function () {
            const id = $(this).data('id');
            window.selectedItems = window.selectedItems.filter(function (x) { return x !== id; });

            window.table3.row($(this).parents('tr')).remove().draw(false);

            window.table3.rows().every(function (rowIdx) {
                this.cell(rowIdx, 0).data(rowIdx + 1);
            });
        });

        // 9) Kosongkan list terpilih
        if (clearBtn) {
            clearBtn.addEventListener('click', function () {
                window.table3.clear().draw(false);
                window.selectedItems = [];
...
<script> // ===== Data Barang (EDIT) =====
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const table2Edit = $('#myTable2Edit').DataTable({
            ...
            scrollCollapse: true,
            info: false,
            paging: true,          // <-- sama: biar tidak limit 10/11
            processing: true,
            language: { url: "../assets/json/id.json" },
            columns: [
        ...
        const clearBtnEdit = document.getElementById('clearTable3Edit');
        window.selectedItemsEdit = window.selectedItemsEdit || [];

        let lastReqEdit = 0;

        async function renderAssetsByPTEdit(pt) {
            const reqId = ++lastReqEdit;

            if (!pt || pt.trim() === '') {
                table2Edit.clear().draw(false);
                return;
            }

            try {
                table2Edit.processing(true);

                const res = await fetch('get_assets_by_pt.php?pt=' + encodeURIComponent(pt), { cache: 'no-store' });
                if (reqId !== lastReqEdit) return;
                ...
                table2Edit.clear();
                table2Edit.rows.add(items);
                table2Edit.draw(false);

                setTimeout(function () {
                    table2Edit.columns.adjust();
                    if (table2Edit.responsive) table2Edit.responsive.recalc();
                }, 50);

            } catch (err) {
                if (reqId !== lastReqEdit) return;
                console.error('❌ load asset edit gagal:', err);
                table2Edit.clear().draw(false);
            } finally {
                try { table2Edit.processing(false); } catch (e) {}
            }
        }

        const lokasiAsalEditEl = document.getElementById('lokasi_asal_edit');
        if (lokasiAsalEditEl) {
            lokasiAsalEditEl.addEventListener('change', function () {
                window.table3Edit.clear().draw(false);
                window.tableBarangMutasiEdit.clear().draw(false);
                window.selectedItemsEdit = [];
                renderAssetsByPTEdit(this.value);
            });
        }

        document.addEventListener('click', function (e) {
            if (e.target.closest('.tombolDataBarangPopupEdit')) {
                const pt = (document.getElementById('lokasi_asal_edit') || {}).value || '';
                renderAssetsByPTEdit(pt);
        ...
        $('#myTable2Edit tbody').on('click', 'tr', function () {
            ...
            const item = table2Edit.row(tr).data();
            if (!item) return;

            const id = item.id_assets;
            if (!id) return;

            if (window.selectedItemsEdit.indexOf(id) !== -1) {
                alert('Barang ini sudah ditambahkan!');
                return;
            }
            window.selectedItemsEdit.push(id);
        ...
        $('#myTable3Edit tbody').on('click', '.delete-row-edit', function () {
            const id = $(this).data('id');
            window.selectedItemsEdit = window.selectedItemsEdit.filter(function (x) { return x !== id; });
...
<script>// fungsi input data barang form input 
    const nb = document.getElementById('nomor_ba');
    if (nb && nb.value !== '') {
    nb.value = nb.value.replace(/\D/g,'').slice(0,3).padStart(3,'0');
    }
...
<script>// fungsi input data barang form edit
    const nbEdit = document.getElementById('nomor_ba_edit');
        if (nbEdit && nbEdit.value !== '') {
        nbEdit.value = nbEdit.value.replace(/\D/g,'').slice(0,3).padStart(3,'0');
    }
...
$(document).ready(function () {
    ...
    openButtons.forEach(btn => {
        btn.addEventListener('click', async function (e) {
            ...
            // === 7️⃣ Tampilkan popup ===
            background.classList.remove('fade-in', 'fade-out')
