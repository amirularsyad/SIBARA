...
$data_karyawan_ho_all = array();
$query_ho_all = $koneksi->query("SELECT nama, posisi, departemen, jabatan
                                 FROM data_karyawan
                                 WHERE dihapus = 0
                                 ORDER BY nama ASC");
if ($query_ho_all) {
    while ($row_ho = $query_ho_all->fetch_assoc()) {
        $data_karyawan_ho_all[] = $row_ho;
    }
}
...
$pt_code_map_json = json_encode($pt_code_map);
$pt_list_json = json_encode($pt_list);
$data_karyawan_ho_all_json = json_encode($data_karyawan_ho_all);
$data_karyawan_test_json = json_encode($data_karyawan_test);
$max_date_today = date('Y-m-d');
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <style>
        ...
        /* =========================
        Layout Data Pengguna
        ========================= */

        .custom-data-pengguna-grid {
            display: grid;
        ...
        .custom-data-pengguna-grid .custom-pengguna-item {
            min-width: 0;
            width: 100%;
        }

        /* biarkan input-group tetap satu baris */
        .custom-data-pengguna-grid .input-group {
            display: flex;
            flex-wrap: nowrap;
            width: 100%;
            margin: 0;
        ...
        .custom-data-pengguna-grid .input-group-text {
            white-space: nowrap;
            min-width: 95px;
            justify-content: flex-start;
        }

        .custom-data-pengguna-grid .form-control,
        .custom-data-pengguna-grid .form-select {
            min-width: 0;
        }

        /* mode HO = 2 baris (2 lalu 1) */
        .custom-data-pengguna-grid.mode-ho .item-peminjam {
            grid-column: 1;
            grid-row: 1;
        }

        .custom-data-pengguna-grid.mode-ho .item-penerima {
            grid-column: 2;
            grid-row: 1;
        }

        .custom-data-pengguna-grid.mode-ho .item-jabatan-peminjam {
            display: none !important;
        }

        .custom-data-pengguna-grid.mode-ho .item-diketahui {
            grid-column: 1;
            grid-row: 2;
        }

        /* mode non HO = 2 baris (2 lalu 2) */
        .custom-data-pengguna-grid.mode-nonho .item-peminjam {
            grid-column: 1;
            grid-row: 1;
        }

        .custom-data-pengguna-grid.mode-nonho .item-jabatan-peminjam {
            grid-column: 2;
            grid-row: 1;
            display: block !important;
        }

        .custom-data-pengguna-grid.mode-nonho .item-penerima {
            grid-column: 1;
            grid-row: 2;
        }

        .custom-data-pengguna-grid.mode-nonho .item-diketahui {
            grid-column: 2;
            grid-row: 2;
        }
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary overflow-x-hidden">
    ...
    <div class="app-wrapper">
            ...
            <div class="sidebar-wrapper">
                <nav class="mt-2">
                    <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="menu" data-accordion="false">
                        ...
                        <li class="nav-item">
                            <a href="../ba_pemutihan/ba_pemutihan.php" class="nav-link">
                                <i class="nav-icon bi bi-newspaper"></i>
                                <p>BA Pemutihan</p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" class="nav-link" aria-disabled="true">
                                ...
                                <p class="text-white">BA Pengembalian</p>
                            </a>
                        </li>
                        <?php if ($ptSekarang == "PT.MSAL (HO)") { ?>
                            <li class="nav-item">
                                <a href="../ba_serah-terima-asset/ba_serah-terima-asset.php" class="nav-link">
                                    <i class="nav-icon bi bi-newspaper"></i>
                                    <p>BA Serah Terima Asset Inventaris</p>
                                </a>
                            </li>
                        <?php } ?>
                        <li class="nav-item">
        ...
        <main id="custom-main" class="custom-main app-main">
            ...
            <section id="table-wrapper" class="table-wrapper bg-white position-relative overflow-visible d-flex flex-column">
                <h2>Daftar Berita Acara Pengembalian Asset Inventaris</h2>
                    ...
                    <table id="myTable" class="table table-bordered table-striped text-center">
                        ...
                        <tbody id="tbody-utama">
                            ...
                            while ($row = $result->fetch_assoc()):
                                ...
                                $stmtPendingEdit = $koneksi->prepare("
                                    SELECT id
                                ...
                                if ($stmtPendingEdit) {
                                    $idBaRow = (int)$row['id'];
                                    $stmtPendingEdit->bind_param("i", $idBaRow);

                                    if ($stmtPendingEdit->execute()) {
                                        $resPendingEdit = $stmtPendingEdit->get_result();
                                        if ($resPendingEdit && $resPendingEdit->num_rows > 0) {
                                            $pendingEditExists = true;
                                        }
                                    }

                                    $stmtPendingEdit->close();
                                }
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <a class="custom-btn-action btn btn-secondary btn-sm btn-detail-ba-pengembalian" href="#" data-id="<?php echo $row['id']; ?>">
                ...
                <div id="popupBoxInput" class="popup-box position-absolute bg-white top-0 rounded-1 flex-column justify-content-start align-items-center p-2" style="height:max-content;align-self:center;z-index:9;width:95%;">
                    ...
                    <form id="formInputPengembalian" class="popupInput d-flex flex-column" method="post" action="proses_simpan.php" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="pengembali" id="input-pengembali-hidden" value="">
                        <input type="hidden" name="jabatan_pengembali" id="jabatan_pengembali_input" value="">
                        <input type="hidden" name="jabatan_penerima" id="jabatan_penerima_input" value="">
                        <input type="hidden" name="jabatan_diketahui" id="jabatan_diketahui_input" value="">

                        <div class="form-section">
                            <div class="row position-relative">
                                <div class="custom-input-form col-8">
                                    <h3>Data Berita Acara Pengembalian</h3>

                                    <div class="row">
                                        <div class="col-md-3 mb-3">
                                            <div class="input-group">
                                                <span class="input-group-text">Tanggal</span>
                                                <input class="form-control" type="date" name="tanggal" id="tanggal" max="<?php echo $max_date_today; ?>" value="<?php echo $tanggal_hari_ini; ?>" required>
                                            </div>
                                        </div>
                                        <div class="col-md-3 mb-3">
                                            <div class="input-group">
                                    ...
                                    <div class="row mt-3 border border-1 p-2 rounded-2 me-1">
                                        ...
                                        <div id="input-data-pengguna-grid" class="custom-data-pengguna-grid mode-ho">
                                            <div class="custom-pengguna-item item-peminjam">
                                                <div id="input-peminjam-select-wrapper" class="input-group d-none">
                                                    <span class="input-group-text">Peminjam</span>
                                                    <select id="input-peminjam-select" class="form-select">
                                                        <option value="">-- Pilih Peminjam --</option>
                                                    </select>
                                                </div>
                                                <div id="input-peminjam-text-wrapper" class="input-group d-none">
                                                    <span class="input-group-text">Peminjam</span>
                                                    <input type="text" id="input-peminjam-text" class="form-control" placeholder="Masukkan nama">
                                                </div>
                                            </div>

                                            <div class="custom-pengguna-item item-jabatan-peminjam">
                                                <div id="input-jabatan-peminjam-wrapper" class="input-group d-none">
                                                    <span class="input-group-text">Jabatan Peminjam</span>
                                                    <input type="text" name="jabatan_peminjam" id="input-jabatan-peminjam-text" class="form-control" placeholder="Masukkan jabatan peminjam">
                                                </div>
                                            </div>

                                            <div class="custom-pengguna-item item-penerima">
                                                <div class="input-group">
                                                    <span class="input-group-text">Penerima</span>
                                                    <select name="penerima" id="input-penerima" class="form-select" required>
                                                        <option value="">-- Pilih Penerima --</option>
                                                    </select>
                                                </div>
                                            </div>

                                            <div class="custom-pengguna-item item-diketahui">
                                                <div class="input-group">
                                                    <span class="input-group-text">Diketahui</span>
                                                    <input type="text" name="diketahui" id="input-diketahui" class="form-control" placeholder="Otomatis dari KTU" required readonly>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="footer-form d-flex w-100 justify-content-between">
                            <h5 class="m-0 mt-3" style="color: darkgray;">*Formulir ini untuk membuat berita acara pengembalian asset inventaris</h5>
                            ...
                            <tbody>
                                ...
                                ?>
                                        <tr class="barang-row"
                                            ...
                                            <td><input type="checkbox" class="form-check-input pilih-barang-checkbox"></td>
                                        </tr>
                                <?php
                                        $no++;
                                    endwhile;
    ...
    <script>
        ...
        var PT_CODE_MAP = <?php echo ($pt_code_map_json ? $pt_code_map_json : '{}'); ?>;
        var PT_LIST = <?php echo ($pt_list_json ? $pt_list_json : '[]'); ?>;
        var DATA_KARYAWAN_HO_ALL = <?php echo ($data_karyawan_ho_all_json ? $data_karyawan_ho_all_json : '[]'); ?>;
        var DATA_KARYAWAN_TEST = <?php echo ($data_karyawan_test_json ? $data_karyawan_test_json : '[]'); ?>;
        var MAX_DATE_TODAY = '<?php echo $max_date_today; ?>';
        ...
        function buildOptionHtml(items, selectedValue, placeholderText) {
            var html = '<option value="">' + escapeHtml(placeholderText || '-- Pilih --') + '</option>';
        ...
        function normalizeBarang(item) {
            return {
                ...
                pt_asal: cleanValue(item.pt_asal, cleanValue(item.pt, '-')),
                ...
                keterangan: cleanLongText(item.keterangan, ''),
                kondisi: cleanValue(item.kondisi, '')
            };
        }
            ...
            if (!arr.length) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    html += '<tr>' +
                        ...
                        '<td>' + escapeHtml(item.pt_asal) + '<input type="hidden" name="barang_pt[]" value="' + escapeHtml(item.pt_asal) + '"></td>' +
                        '<td>' + escapeHtml(item.po) + '<input type="hidden" name="barang_po[]" value="' + escapeHtml(item.po) + '"></td>' +
                        '<td>' + escapeHtml(item.coa) + '<input type="hidden" name="barang_coa[]" value="' + escapeHtml(item.coa) + '"></td>' +
                        '<td>' + escapeHtml(item.kode_assets) + '<input type="hidden" name="barang_kode_assets[]" value="' + escapeHtml(item.kode_assets) + '"></td>' +
                        '<td>' + escapeHtml(item.merk) + '<input type="hidden" name="barang_merk[]" value="' + escapeHtml(item.merk) + '"></td>' +
                        '<td>' + escapeHtml(item.sn) + '<input type="hidden" name="barang_sn[]" value="' + escapeHtml(item.sn) + '"></td>' +
                        '<td>' + escapeHtml(item.user) + '<input type="hidden" name="barang_user[]" value="' + escapeHtml(item.user) + '"></td>' +
                        '<td>' + formatRupiah(item.harga_beli) + '<input type="hidden" name="barang_harga_beli[]" value="' + escapeHtml(item.harga_beli) + '"></td>' +
                        '<td>' + escapeHtml(item.tahun_perolehan) + '<input type="hidden" name="barang_tahun_perolehan[]" value="' + escapeHtml(item.tahun_perolehan) + '"></td>' +
                        '<td style="min-width:220px;">' +
                        '<textarea name="barang_keterangan[]" class="form-control form-control-sm" rows="2" required oninput="updateBarangField(\'' + mode + '\',' + i + ',\'keterangan\', this.value)">' + escapeHtml(item.keterangan) + '</textarea>' +
                        '</td>' +
                        '<td style="min-width:180px;">' +
                        '<select name="barang_kondisi[]" class="form-select form-select-sm" required onchange="updateBarangField(\'' + mode + '\',' + i + ',\'kondisi\', this.value)">' +
        ...
        function getPreferredJabatan(item) {
            if (!item) {
                return '-';
            }
            return item.posisi ? item.posisi : (item.jabatan ? item.jabatan : '-');
        }

        function getPeminjamCandidates(selectedPT) {
            var items = [];
            if (isHoPT(selectedPT)) {
                for (var i = 0; i < DATA_KARYAWAN_HO_ALL.length; i++) {
                    items.push({
                        nama: DATA_KARYAWAN_HO_ALL[i].nama,
                        jabatan_value: getPreferredJabatan(DATA_KARYAWAN_HO_ALL[i])
                    });
                }
            }
            return items;
        ...
        function normalizeNamaUntukCompare(val) {
            return $.trim(String(val || '')).toLowerCase();
        }

        function getPenerimaCandidates(selectedPT, pengembaliDipilih) {
            var items = [];
            var i;
            var namaPengembali = normalizeNamaUntukCompare(pengembaliDipilih);

            if (isHoPT(selectedPT)) {
                for (i = 0; i < DATA_KARYAWAN_HO_ALL.length; i++) {
                    if (
                        String(DATA_KARYAWAN_HO_ALL[i].departemen) === 'MIS' &&
                        String(DATA_KARYAWAN_HO_ALL[i].jabatan) === 'Staf' &&
                        normalizeNamaUntukCompare(DATA_KARYAWAN_HO_ALL[i].nama) !== namaPengembali
                    ) {
                        items.push({
                            nama: DATA_KARYAWAN_HO_ALL[i].nama,
                            jabatan_value: getPreferredJabatan(DATA_KARYAWAN_HO_ALL[i])
                ...
                for (i = 0; i < DATA_KARYAWAN_TEST.length; i++) {
                    if (
                        ptMatches(DATA_KARYAWAN_TEST[i].pt, selectedPT) &&
                        String(DATA_KARYAWAN_TEST[i].posisi) === 'IT Support' &&
                        normalizeNamaUntukCompare(DATA_KARYAWAN_TEST[i].nama) !== namaPengembali
                    ) {
                        items.push({
                            nama: DATA_KARYAWAN_TEST[i].nama,
                            jabatan_value: DATA_KARYAWAN_TEST[i].posisi
                        });
                    }
                }
            }
        ...
        function getAutoDiketahui(selectedPT) {
            var i;

            if (isHoPT(selectedPT)) {
                for (i = 0; i < DATA_KARYAWAN_HO_ALL.length; i++) {
                    if (
                        String(DATA_KARYAWAN_HO_ALL[i].jabatan) === 'Dept. Head' &&
                        String(DATA_KARYAWAN_HO_ALL[i].departemen) === 'MIS'
                    ) {
                        return {
                            nama: DATA_KARYAWAN_HO_ALL[i].nama,
                            jabatan_value: 'Dept. Head MIS'
                        };
                    }
                }
            ...
            for (i = 0; i < DATA_KARYAWAN_TEST.length; i++) {
                var posisi = $.trim(String(DATA_KARYAWAN_TEST[i].posisi || ''));
                if (
                    ptMatches(DATA_KARYAWAN_TEST[i].pt, selectedPT) &&
                    posisi.toUpperCase() === 'KTU'
                ) {
                    return {
                        nama: DATA_KARYAWAN_TEST[i].nama,
                        jabatan_value: posisi !== '' ? posisi : 'KTU'
                    };
                }
            }
        ...
        function getFormContext(mode) {
            if (mode === 'edit') {
                return {
                    pt: $('#edit-pt').val(),
                    ptSelect: $('#edit-pt'),
                    penggunaGrid: $('#edit-data-pengguna-grid'),
                    peminjamSelectWrapper: $('#edit-peminjam-select-wrapper'),
                    peminjamTextWrapper: $('#edit-peminjam-text-wrapper'),
                    peminjamSelect: $('#edit-peminjam-select'),
                    peminjamText: $('#edit-peminjam-text'),
                    jabatanPeminjamWrapper: $('#edit-jabatan-peminjam-wrapper'),
                    jabatanPeminjamText: $('#edit-jabatan-peminjam-text'),
                    pengembaliHidden: $('#edit-pengembali-hidden'),
                    jabatanPengembali: $('#jabatan_pengembali_edit'),
                    penerimaSelect: $('#edit-penerima'),
                    jabatanPenerima: $('#jabatan_penerima_edit'),
                    diketahuiInput: $('#edit-diketahui'),
                    jabatanDiketahuiHidden: $('#jabatan_diketahui_edit')
                };
            }
            ...
            return {
                pt: $('#perusahaan').val(),
                ptSelect: $('#perusahaan'),
                penggunaGrid: $('#input-data-pengguna-grid'),
                peminjamSelectWrapper: $('#input-peminjam-select-wrapper'),
                peminjamTextWrapper: $('#input-peminjam-text-wrapper'),
                peminjamSelect: $('#input-peminjam-select'),
                peminjamText: $('#input-peminjam-text'),
                jabatanPeminjamWrapper: $('#input-jabatan-peminjam-wrapper'),
                jabatanPeminjamText: $('#input-jabatan-peminjam-text'),
                pengembaliHidden: $('#input-pengembali-hidden'),
                jabatanPengembali: $('#jabatan_pengembali_input'),
                penerimaSelect: $('#input-penerima'),
                jabatanPenerima: $('#jabatan_penerima_input'),
                diketahuiInput: $('#input-diketahui'),
                jabatanDiketahuiHidden: $('#jabatan_diketahui_input')
            };
        }
        ...
        function syncPengembaliHidden(mode) {
            var ctx = getFormContext(mode);

            if (isHoPT(ctx.pt)) {
                ctx.pengembaliHidden.val(ctx.peminjamSelect.val());
                updateJabatanHidden(ctx.peminjamSelect, ctx.jabatanPengembali);

                if (ctx.jabatanPeminjamText.length) {
                    ctx.jabatanPeminjamText.val('');
                }
            } else {
                ctx.pengembaliHidden.val(ctx.peminjamText.val());
                ctx.jabatanPengembali.val($.trim(ctx.jabatanPeminjamText.val()));
            }
        }

        function populatePengembalianActors(mode, presetPengembali, presetPenerima, presetJabatanPengembali, presetJabatanPenerima, presetDiketahui, presetJabatanDiketahui) {
            var ctx = getFormContext(mode);
            var selectedPT = ctx.pt;
            var isHO;
            var currentPengembali;
            var currentPenerima;
            var currentJabatanPengembali;
            var autoDiketahui;
            var autoNama;
            var autoJabatan;

            if (!selectedPT) {
                ctx.peminjamSelectWrapper.addClass('d-none');
                ctx.peminjamTextWrapper.addClass('d-none');
                ctx.jabatanPeminjamWrapper.addClass('d-none');

                if (ctx.penggunaGrid.length) {
                    ctx.penggunaGrid.removeClass('mode-ho mode-nonho');
                }

                ctx.peminjamSelect.prop('required', false).html('<option value="">-- Pilih Peminjam --</option>');
                ctx.peminjamText.prop('required', false).val('');
                ctx.jabatanPeminjamText.prop('required', false).val('');

                ctx.penerimaSelect.html('<option value="">-- Pilih Penerima --</option>');
                ctx.pengembaliHidden.val('');
                ctx.jabatanPengembali.val('');
                ctx.jabatanPenerima.val('');
                ctx.diketahuiInput.val('').prop('readonly', false);
                ctx.jabatanDiketahuiHidden.val('');

                if (ctx.diketahuiInput.length && ctx.diketahuiInput[0].setCustomValidity) {
                    ctx.diketahuiInput[0].setCustomValidity('');
                }
                return;
            }

            isHO = isHoPT(selectedPT);

            if (ctx.penggunaGrid.length) {
                ctx.penggunaGrid.removeClass('mode-ho mode-nonho');
                ctx.penggunaGrid.addClass(isHO ? 'mode-ho' : 'mode-nonho');
            }
            currentPengembali = (typeof presetPengembali !== 'undefined' && presetPengembali !== null) ? presetPengembali : ctx.pengembaliHidden.val();
            currentPenerima = (typeof presetPenerima !== 'undefined' && presetPenerima !== null) ? presetPenerima : ctx.penerimaSelect.val();
            currentJabatanPengembali = (typeof presetJabatanPengembali !== 'undefined' && presetJabatanPengembali !== null)
                ? presetJabatanPengembali
                : (ctx.jabatanPeminjamText.length && $.trim(ctx.jabatanPeminjamText.val()) !== '' ? ctx.jabatanPeminjamText.val() : ctx.jabatanPengembali.val());

            if (isHO) {
                ctx.peminjamSelectWrapper.removeClass('d-none');
                ctx.peminjamTextWrapper.addClass('d-none');
                ctx.jabatanPeminjamWrapper.addClass('d-none');

                ctx.peminjamText.prop('required', false).val('');
                ctx.jabatanPeminjamText.prop('required', false).val('');
                ctx.peminjamSelect.prop('required', true);

                ctx.peminjamSelect.html(buildOptionHtml(getPeminjamCandidates(selectedPT), currentPengembali, '-- Pilih Peminjam --'));
                if (currentPengembali) {
                    ctx.peminjamSelect.val(currentPengembali);
                }
                ...
                ctx.peminjamSelectWrapper.addClass('d-none');
                ctx.peminjamTextWrapper.removeClass('d-none');
                ctx.jabatanPeminjamWrapper.removeClass('d-none');

                ctx.peminjamSelect.prop('required', false).html('<option value="">-- Pilih Peminjam --</option>');
                ctx.peminjamText.prop('required', true).val(currentPengembali ? currentPengembali : '');

                ctx.jabatanPeminjamText
                    .prop('required', true)
                    .val(currentJabatanPengembali ? currentJabatanPengembali : '');

                ctx.pengembaliHidden.val(ctx.peminjamText.val());
                ctx.jabatanPengembali.val($.trim(ctx.jabatanPeminjamText.val()));
            }

            currentPengembali = isHO ? ctx.peminjamSelect.val() : ctx.peminjamText.val();

            ctx.penerimaSelect.html(buildOptionHtml(getPenerimaCandidates(selectedPT, currentPengembali), currentPenerima, '-- Pilih Penerima --'));
            if (currentPenerima) {
                ctx.penerimaSelect.val(currentPenerima);
            }

            if (normalizeNamaUntukCompare(ctx.penerimaSelect.val()) === normalizeNamaUntukCompare(currentPengembali)) {
                ctx.penerimaSelect.val('');
            }

            updateJabatanHidden(ctx.penerimaSelect, ctx.jabatanPenerima);
            if (ctx.penerimaSelect.val() && presetJabatanPenerima && $.trim(presetJabatanPenerima) !== '') {
                ctx.jabatanPenerima.val(presetJabatanPenerima);
            }
            if (!ctx.penerimaSelect.val()) {
                ctx.jabatanPenerima.val('');
            }

            autoDiketahui = getAutoDiketahui(selectedPT);
            autoNama = $.trim(autoDiketahui.nama ? autoDiketahui.nama : '');
            autoJabatan = $.trim(autoDiketahui.jabatan_value ? autoDiketahui.jabatan_value : '');

            if (!isHO && autoNama === '' && typeof presetDiketahui !== 'undefined' && presetDiketahui !== null && $.trim(String(presetDiketahui)) !== '') {
                autoNama = $.trim(String(presetDiketahui));
            }

            if (!isHO && autoJabatan === '' && typeof presetJabatanDiketahui !== 'undefined' && presetJabatanDiketahui !== null && $.trim(String(presetJabatanDiketahui)) !== '') {
                autoJabatan = $.trim(String(presetJabatanDiketahui));
            }

            ctx.diketahuiInput.val(autoNama);
            ctx.diketahuiInput.prop('readonly', true);
            ctx.jabatanDiketahuiHidden.val(autoJabatan !== '' ? autoJabatan : (isHO ? 'Dept. Head MIS' : 'KTU'));

            if (ctx.diketahuiInput.length && ctx.diketahuiInput[0].setCustomValidity) {
                if (!isHO && autoNama === '') {
                    ctx.diketahuiInput[0].setCustomValidity('Data KTU untuk PT ini belum tersedia.');
                } else {
                    ctx.diketahuiInput[0].setCustomValidity('');
                }
            }

            syncPengembaliHidden(mode);
        }
    </script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (ptInput) {
                ptInput.addEventListener('change', function() {
                    ...
                    updateNomorBA();
                    populatePengembalianActors('input');
            ...
            $('#input-peminjam-select').on('change', function() {
                populatePengembalianActors(
                    'input',
                    $(this).val(),
                    $('#input-penerima').val(),
                    $('#jabatan_pengembali_input').val(),
                    $('#jabatan_penerima_input').val(),
                    $('#input-diketahui').val(),
                    $('#jabatan_diketahui_input').val()
                );
            });

            $('#input-peminjam-text').on('input', function() {
                populatePengembalianActors(
                    'input',
                    $(this).val(),
                    $('#input-penerima').val(),
            ...
            $('#input-penerima').on('change', function() {
                updateJabatanHidden($('#input-penerima'), $('#jabatan_penerima_input'));
            });

            $('#input-jabatan-peminjam-text').on('input', function() {
                syncPengembaliHidden('input');
            });
        });
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderEditForm(data, barangList, gambarList) {
                ...
                for (var i = 0; i < gambarList.length; i++) {
                    ...
                    gambarHTML += '' +
                        ...
                        '   <img src="' + escapeHtml(g.file_path ? g.file_path : '') + '" style="display:block;">' +
                ...
                body.innerHTML = '' +
                    '<form id="formEditPengembalian" class="popupEdit d-flex flex-column" method="post" action="proses_edit.php" enctype="multipart/form-data">' +
                    ...
                    '   <input type="hidden" name="pengembali" id="edit-pengembali-hidden" value="' + escapeHtml(data.pengembali ? data.pengembali : '') + '">' +
                    '   <input type="hidden" name="jabatan_pengembali" id="jabatan_pengembali_edit" value="' + escapeHtml(data.jabatan_pengembali ? data.jabatan_pengembali : '') + '">' +
                    '   <input type="hidden" name="jabatan_penerima" id="jabatan_penerima_edit" value="' + escapeHtml(data.jabatan_penerima ? data.jabatan_penerima : '') + '">' +
                    '   <input type="hidden" name="jabatan_diketahui" id="jabatan_diketahui_edit" value="' + escapeHtml(data.jabatan_diketahui ? data.jabatan_diketahui : '') + '">' +
                    ...
                    '                       <div class="input-group">' +
                    '                           <span class="input-group-text">Tanggal</span>' +
                    '                           <input class="form-control" type="date" name="tanggal" id="tanggal_edit" max="' + escapeHtml(MAX_DATE_TODAY) + '" value="' + escapeHtml(data.tanggal ? data.tanggal : '') + '" required>' +
                    '                       </div>' +
                    '                   </div>' +
                    '                   <div class="col-md-3 mb-3">' +
                    '                       <div class="input-group">' +
                    ...
                    '                               <option value="">-- Pilih PT --</option>' + buildEditPtOptions(data.pt) +
                    '                           </select>' +
                    '                       </div>' +
                    '                   </div>' +
                    '               </div>' +
                    ...
                    '                                   <thead>' +
                    '                                       <tr>' +
                    '                                           <th>No</th><th>PT Asal</th><th>PO</th><th>COA</th><th>Kode Asset</th><th>Merek</th><th>SN</th><th>User</th><th>Harga Beli</th><th>Tahun Perolehan</th><th>Keterangan</th><th>Kondisi</th><th>Hapus</th>' +
                    '                                       </tr>' +
                    '                                   </thead>' +
                    ...
                    '                   <div id="edit-data-pengguna-grid" class="custom-data-pengguna-grid mode-ho">' +
                    '                       <div class="custom-pengguna-item item-peminjam">' +
                    '                           <div id="edit-peminjam-select-wrapper" class="input-group d-none">' +
                    '                               <span class="input-group-text">Peminjam</span>' +
                    '                               <select id="edit-peminjam-select" class="form-select">' +
                    '                                   <option value="">-- Pilih Peminjam --</option>' +
                    '                               </select>' +
                    '                           </div>' +
                    '                           <div id="edit-peminjam-text-wrapper" class="input-group d-none">' +
                    '                               <span class="input-group-text">Peminjam</span>' +
                    '                               <input type="text" id="edit-peminjam-text" class="form-control" placeholder="Masukkan nama peminjam">' +
                    '                           </div>' +
                    '                       </div>' +
                    '                       <div class="custom-pengguna-item item-jabatan-peminjam">' +
                    '                           <div id="edit-jabatan-peminjam-wrapper" class="input-group d-none">' +
                    '                               <span class="input-group-text">Jabatan Peminjam</span>' +
                    '                               <input type="text" name="jabatan_peminjam" id="edit-jabatan-peminjam-text" class="form-control" placeholder="Masukkan jabatan peminjam" value="' + escapeHtml(data.jabatan_pengembali ? data.jabatan_pengembali : '') + '">' +
                    '                           </div>' +
                    '                       </div>' +
                    '                       <div class="custom-pengguna-item item-penerima">' +
                    '                           <div class="input-group">' +
                    '                               <span class="input-group-text">Penerima</span>' +
                    '                               <select name="penerima" id="edit-penerima" class="form-select" required>' +
                    '                                   <option value="">-- Pilih Penerima --</option>' +
                    '                               </select>' +
                    '                           </div>' +
                    '                       </div>' +
                    ...
                    '                           <div class="input-group">' +
                    '                               <span class="input-group-text">Diketahui</span>' +
                    '                               <input type="text" name="diketahui" id="edit-diketahui" class="form-control" required readonly>' +
                    '                           </div>' +
                    '                       </div>' +
                    '                   </div>' +
                    '               </div>' +
                ...
                for (var b = 0; b < barangList.length; b++) {
                    normalizedBarang.push(normalizeBarang({
                        ...
                        pt_asal: barangList[b].pt_asal ? barangList[b].pt_asal : barangList[b].pt,
                        po: barangList[b].po,
                        ...
                        keterangan: barangList[b].keterangan,
                ...
                populatePengembalianActors(
                    'edit',
                    data.pengembali ? data.pengembali : '',
                    data.penerima ? data.penerima : '',
                    data.jabatan_pengembali ? data.jabatan_pengembali : '',
                    data.jabatan_penerima ? data.jabatan_penerima : '',
                    data.diketahui ? data.diketahui : '',
                    data.jabatan_diketahui ? data.jabatan_diketahui : ''
                );

                $('#edit-pt').off('change').on('change', function() {
                    setIdPtBySelect('edit-pt', 'id_pt_edit');
                    populatePengembalianActors('edit');
                    if ($.fn.DataTable.isDataTable('#myTable2')) {
                        $('#myTable2').DataTable().draw();
                    }
                });

                $('#edit-peminjam-select').off('change').on('change', function() {
                    populatePengembalianActors(
                        'edit',
                        $(this).val(),
                        $('#edit-penerima').val(),
                        $('#jabatan_pengembali_edit').val(),
                        $('#jabatan_penerima_edit').val(),
                        $('#edit-diketahui').val(),
                        $('#jabatan_diketahui_edit').val()
                    );
                });

                $('#edit-peminjam-text').off('input').on('input', function() {
                    populatePengembalianActors(
                        'edit',
                        $(this).val(),
                        $('#edit-penerima').val(),
                ...
                $('#edit-penerima').off('change').on('change', function() {
                    updateJabatanHidden($('#edit-penerima'), $('#jabatan_penerima_edit'));
                });

                $('#edit-jabatan-peminjam-text').off('input').on('input', function() {
                    syncPengembaliHidden('edit');
                });
            }
            ...
            $(document).on('click', '.tombolPopupEdit', function(e) {
                ...
                fetch('get_edit_ba_pengembalian_v2.php?id=' + encodeURIComponent(id), {
                        cache: 'no-store'
                    })
                    .then(function(resp) {
                        if (!resp.ok) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function buildApprovalTable(data) {
                var actors = [{
                        label: 'Peminjam',
                        nama: data.pengembali ? data.pengembali : '-',
                        jabatan: data.jabatan_pengembali ? data.jabatan_pengembali : '-',
                        approval: data.approval_1
                    },
                    {
                        label: 'Penerima',
                        nama: data.penerima ? data.penerima : '-',
                        jabatan: data.jabatan_penerima ? data.jabatan_penerima : '-',
                        approval: data.approval_2
                    },
                    {
                        label: 'Diketahui',
                        nama: data.diketahui ? data.diketahui : '-',
                        jabatan: data.jabatan_diketahui ? data.jabatan_diketahui : '-',
                ...
                for (var i = 0; i < actors.length; i++) {
                    ...
                    jabatan += '<td>' + escapeHtml(actors[i].jabatan) + '</td>';
            ...
            $(document).on('click', '.btn-detail-ba-pengembalian', function(e) {
                e.preventDefault();
                var id = $(this).data('id');
                if (!id) {
                    return;
                }
                fetch('get_detail_ba_pengembalian_v2.php?id=' + encodeURIComponent(id), {
                        cache: 'no-store'
                    })
                    .then(function(resp) {
                        if (!resp.ok) {
                    ...
                    .then(function(res) {
                        ...
                        var ptCode = PT_CODE_MAP[data.pt] ? PT_CODE_MAP[data.pt] : '-';

                        var html = '<h2>Detail Data Pengembalian ' + escapeHtml(data.nomor_ba ? data.nomor_ba : '-') + '/' + escapeHtml(ptCode) + '-MIS/' + escapeHtml(romawi) + '/' + escapeHtml(tahun) + '</h2>';
                        ...
                        html += '' +
                            ...
                            '               <thead>' +
                            '                   <tr><th>No</th><th>PT Asal</th><th>PO</th><th>COA</th><th>Kode Asset</th><th>Merk</th><th>SN</th><th>User</th><th>Harga Beli</th><th>Tahun Perolehan</th><th>Keterangan</th><th>Kondisi</th></tr>' +
                            '               </thead>' +
                            '               <tbody>';
                        ...
                        if (barangList.length) {
                            for (var i = 0; i < barangList.length; i++) {
                                html += '<tr>' +
                                    '<td>' + (i + 1) + '</td>' +
                                    '<td>' + escapeHtml(barangList[i].pt ? barangList[i].pt : (barangList[i].pt_asal ? barangList[i].pt_asal : '-')) + '</td>' +
                                    ...
                                    '<td>' + escapeHtml(barangList[i].keterangan ? barangList[i].keterangan : '-') + '</td>' +
