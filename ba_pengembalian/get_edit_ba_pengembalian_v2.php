...
try {
    ...
    // DATA UTAMA
    // =============================
    $stmt = $koneksi->prepare("SELECT * FROM berita_acara_pengembalian_v2 WHERE id = ? AND dihapus = 0 LIMIT 1");
    if (!$stmt) {
    ...
    if ($resB) {
        while ($rowB = $resB->fetch_assoc()) {
            $barangList[] = array(
                ...
                'pt' => (isset($rowB['pt']) && trim($rowB['pt']) !== '') ? $rowB['pt'] : '-',
                ...
                'keterangan' => (isset($rowB['keterangan']) && trim($rowB['keterangan']) !== '') ? $rowB['keterangan'] : ''
            );
        }
    }
    $stmtB->close();
    ...
    $headerMap = array(
        ...
        'pt' => 'PT',
        'pengembali' => 'Peminjam',
        'jabatan_pengembali' => 'Jabatan Peminjam',
        'penerima' => 'Penerima',
        'jabatan_penerima' => 'Jabatan Penerima',
        'diketahui' => 'Diketahui',
        'jabatan_diketahui' => 'Jabatan Diketahui'
    );
    ...
    error_log('get_edit_ba_pengembalian error: ' . $e->getMessage());
