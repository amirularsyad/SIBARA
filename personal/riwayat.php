...
if ($filterJenisBA === '' || $filterJenisBA === 'kerusakan') {
    $sql .= "(SELECT id, tanggal, nomor_ba,
            ...
            WHERE ((nama_aprv1 = ? AND approval_1 = 1) 
                OR (nama_aprv2 = ? AND approval_2 = 1))
                AND dihapus = 0
            $whereKerusakan)";
}
...
if ($filterJenisBA === '' || $filterJenisBA === 'pengembalian') {
    $sql .= "(SELECT id, tanggal, nomor_ba,
            ...
            WHERE ((nama_pengembali = ? AND approval_1 = 1) 
                OR (nama_penerima = ? AND approval_2 = 1)
                OR (SUBSTRING_INDEX(diketahui, ' - ', 1) = ? AND approval_3 = 1))
                AND dihapus = 0
            $wherePengembalian)";
}
...
<body  class="layout-fixed sidebar-expand-lg bg-body-tertiary overflow-x-hidden">
    ...
    <aside class="app-sidebar shadow" data-bs-theme="dark"> <!-- Sidebar -->
        ...
        <nav class="mt-2">
            ...
            <!-- <li class="nav-item">
                <a href="#" class="nav-link">
                <i class="nav-icon bi bi-clipboard2-data text-white"></i>
                <p class="text-white">
                    Riwayat Approval
                </p>
                </a>
    ...
    <section class="table-wrapper bg-white position-relative overflow-visible">
        <?php 
        // echo htmlspecialchars($_SESSION['nama'] ?? 'Anda'); 
        ?>
        ...
        <?php endif; ?>
        <h2>Daftar Riwayat Approval BA Anda</h2>
            ...
            <tbody>
                ...
                while ($row = $result->fetch_assoc()):
                    ...
                    if ($jenis_ba === 'kerusakan') {
                        if ($role_name1 === $namaUser && $aprv1 === 1) {
                            $peran = 'Pembuat';
                            $approval_status = $aprv1;
                            $approval_field = 1;
                        } elseif ($role_name2 === $namaUser && $aprv2 === 1) {
                            $peran = 'Diketahui';
                            $approval_status = $aprv2;
                            $approval_field = 2;
                        } elseif ($role_name3 === $namaUser && $aprv3 === 1) { 
                    ...
                    if ($jenis_ba === 'kerusakan') {
                        ...
                        $surat_url = "surat_output_kerusakan.php?id=" . intval($row['id']);
                    } else {
