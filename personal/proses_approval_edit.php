...
if ($aksi === "tolak") {
    ...
    if ($jenisBA === "pengembalian") {
        ...
        // 1. UPDATE DATA LAMA BA: pending_status = 0
        $upd_lama = "
        ...
        // 2. AMBIL DATA PERUBAHAN BA (status = 1)
        $sql = "
            SELECT * FROM history_n_temp_ba_pengembalian_v2
            WHERE id_ba = $id_ba
              AND pending_approver = '" . addslashes($approver) . "'
              AND pending_status = 1
              AND status = 1
        ...
        if ($hasil && $hasil->num_rows > 0) {
            ...
            // 3. Escape semua data BA Pengembalian
            $fields = array(
                'tanggal',
                'nomor_ba',
                'nama_pembuat',
            ...
            foreach ($fields as $f) {
                if (
                    ...
                    if (is_null($data[$f])) {
                        ...
                        $escaped[$f] = "'" . addslashes($data[$f]) . "'";
                    }
                }
            }

            // 4. UPDATE ke tabel utama BA Pengembalian
            $update = "
                UPDATE berita_acara_pengembalian_v2 SET
                    nomor_ba = {$escaped['nomor_ba']},
                    tanggal = {$escaped['tanggal']},
                    nama_pembuat = {$escaped['nama_pembuat']},
                    pt = {$escaped['pt']},
                    id_pt = {$escaped['id_pt']},
                    pengembali = {$escaped['pengembali']},
                    jabatan_pengembali = {$escaped['jabatan_pengembali']},
                    penerima = {$escaped['penerima']},
                    jabatan_penerima = {$escaped['jabatan_penerima']},
                    ...
                    dihapus = {$escaped['dihapus']},
                    pending_hapus = {$escaped['pending_hapus']},
                    pending_hapus_approver = {$escaped['pending_hapus_approver']}
                WHERE id = $id_ba
            ";
        ...
        // 5. HAPUS HISTORY BA YANG SUDAH DITERAPKAN
        $del = "
            DELETE FROM history_n_temp_ba_pengembalian_v2
            WHERE id_ba = $id_ba
              AND pending_approver = '" . addslashes($approver) . "'
              AND pending_status = 1
              AND status = 1
        ...
        // ==============================================================
        // === BAGIAN BARANG PENGEMBALIAN ================================
        // ==============================================================

        // 7. UPDATE DATA BARANG LAMA: pending_status = 0
        $upd_lama_barang = "
            ...
            WHERE id_ba = $id_ba
              ...
              AND status = 0
        ";
        if (!$koneksi->query($upd_lama_barang)) $all_ok = false;

        // 8. HAPUS DATA BARANG ASLI
        $delete_barang = "
            DELETE FROM barang_pengembalian_v2
            WHERE id_ba = $id_ba
        ";
        if (!$koneksi->query($delete_barang)) {
            $all_ok = false;
        }

        if ($all_ok) {

            // 9. AMBIL DATA BARANG BARU (status = 1)
            $sql_barang = "
                SELECT * FROM history_n_temp_barang_pengembalian_v2
                WHERE id_ba = $id_ba
                  AND pending_status = 1
                  AND status = 1
                ORDER BY id ASC
                ...
                while ($data_barang = $hasil_barang->fetch_assoc()) {
                    ...
                    foreach ($fields_barang as $f) {
                        if (
                            ...
                            if (is_null($data_barang[$f])) {
                                $escaped_barang[$f] = "NULL";
                            } else {
                    ...
                    $insert = "
                        INSERT INTO barang_pengembalian_v2 (
                            id_ba,
                            pt,
                            id_pt,
                            po,
                            ...
                            {$escaped_barang['sn']},
                            {$escaped_barang['user']},
                            {$escaped_barang['tahun_perolehan']},
                            {$escaped_barang['harga_beli']},
                            {$escaped_barang['kondisi']},
                            {$escaped_barang['keterangan']},
    ...
    if ($jenisBA === "pemutihan") {

        // 1. DELETE data lama BA
        if (!$koneksi->query("
            DELETE FROM history_n_temp_ba_pemutihan
            WHERE id_ba = $id_ba
            AND pending_approver = '$approver'
            AND pending_status = 1
            AND status = 0
        ...
        // 2. UPDATE data baru BA
        if (!$koneksi->query("
            UPDATE history_n_temp_ba_pemutihan 
            SET pending_status = 2,
                status = 2,
                pending_approver = '',
                alasan_tolak = '$alasan'
        ...
        // 4. DELETE data lama barang
        if (!$koneksi->query("
            DELETE FROM history_n_temp_barang_pemutihan
            WHERE id_ba = $id_ba
                AND pending_status = 1
                AND status = 0
        ")) {
        ...
        // 5. UPDATE data baru barang
        if (!$koneksi->query("
            UPDATE history_n_temp_barang_pemutihan 
            SET pending_status = 2,
                status = 2,
                pending_approver = ''
            WHERE id_ba = $id_ba
    ...
    if ($jenisBA !== "kerusakan" && $jenisBA !== "pengembalian" && $jenisBA !== "mutasi" && $jenisBA !== "st_asset" && $jenisBA !== "pemutihan") {
...
if ($aksi === "setuju") {
    ...
    if ($jenisBA === "pengembalian") {
        ...
        if ($hasil && $hasil->num_rows > 0) {
            ...
            // 3. Escape field BA Pengembalian
            $fields = array(
                'tanggal',
                'nomor_ba',
                'pt',
            ...
            $escaped = array();
            foreach ($fields as $f) {
                if ($f === 'id_pt' || strpos($f, 'approval_') !== false) {
                    $escaped[$f] = (int)$data[$f];
                } else {
                    $escaped[$f] = "'" . addslashes($data[$f]) . "'";
                }
            ...
            // 4. UPDATE tabel utama BA Pengembalian
            $update = "
                UPDATE berita_acara_pengembalian_v2 SET
                    tanggal = {$escaped['tanggal']},
                    nomor_ba = {$escaped['nomor_ba']},
        ...
        if ($all_ok) {
            ...
            if ($hasil_barang && $hasil_barang->num_rows > 0) :
                ...
                while ($data_barang = $hasil_barang->fetch_assoc()) {
                    ...
                    $insert = "
                        INSERT INTO barang_pengembalian_v2 (
                            id_ba, id_pt, pt, po, coa,
                            kode_assets, merk, sn, keterangan, kondisi
                        ) VALUES (
                            {$escaped_barang['id_ba']},
                            {$escaped_barang['id_pt']},
                            {$escaped_barang['pt']},
    ...
    if ($jenisBA === "mutasi") {
        ...
        if ($hasil && $hasil->num_rows > 0) {
            ...
            $fields = [
                ...
                'jabatan_pengirim2',
                'hrd_ga_pengirim',
                'jabatan_hrd_ga_pengirim',
                'penerima1',
                'jabatan_penerima1',
                'penerima2',
                'jabatan_penerima2',
                'hrd_ga_penerima',
                'jabatan_hrd_ga_penerima',
                'diketahui',
                'jabatan_diketahui',
                'pemeriksa1',
                'jabatan_pemeriksa1',
    ...
    if ($jenisBA === "pemutihan") {
        ...
        // 2. AMBIL DATA PERUBAHAN BA (status = 1)
        $sql = "
        SELECT * FROM history_n_temp_ba_pemutihan
        WHERE id_ba = $id_ba
          AND pending_approver = '" . addslashes($approver) . "'
          AND pending_status = 1
          AND status = 1
        ...
        if ($hasil && $hasil->num_rows > 0) {
            ...
            $update = "
            UPDATE berita_acara_pemutihan SET
                tanggal = {$escaped['tanggal']},
                nomor_ba = {$escaped['nomor_ba']},
                pt = {$escaped['pt']},
                id_pt = {$escaped['id_pt']},
                ...
                pembuat = {$escaped['pembuat']},
                jabatan_pembuat = {$escaped['jabatan_pembuat']},
                pemeriksa = {$escaped['pemeriksa']},
                jabatan_pemeriksa = {$escaped['jabatan_pemeriksa']},

                diketahui1 = {$escaped['diketahui1']},
                jabatan_diketahui1 = {$escaped['jabatan_diketahui1']},
                diketahui2 = {$escaped['diketahui2']},
                jabatan_diketahui2 = {$escaped['jabatan_diketahui2']},
                diketahui3 = {$escaped['diketahui3']},
                jabatan_diketahui3 = {$escaped['jabatan_diketahui3']},
                dibukukan = {$escaped['dibukukan']},
                jabatan_dibukukan = {$escaped['jabatan_dibukukan']},
                disetujui1 = {$escaped['disetujui1']},
                jabatan_disetujui1 = {$escaped['jabatan_disetujui1']},
                disetujui2 = {$escaped['disetujui2']},
                jabatan_disetujui2 = {$escaped['jabatan_disetujui2']},
                disetujui3 = {$escaped['disetujui3']},
                jabatan_disetujui3 = {$escaped['jabatan_disetujui3']},

                pembuat_site = {$escaped['pembuat_site']},
                jabatan_pembuat_site = {$escaped['jabatan_pembuat_site']},
                pemeriksa_site = {$escaped['pemeriksa_site']},
                jabatan_pemeriksa_site = {$escaped['jabatan_pemeriksa_site']},
                diketahui1_site = {$escaped['diketahui1_site']},
                jabatan_diketahui1_site = {$escaped['jabatan_diketahui1_site']},
                disetujui1_site = {$escaped['disetujui1_site']},
                jabatan_disetujui1_site = {$escaped['jabatan_disetujui1_site']},
        ...
        if ($all_ok) {
            $delete_barang = "
            DELETE FROM barang_pemutihan
            WHERE id_ba = $id_ba
        ";
            if (!$koneksi->query($delete_barang)) $all_ok = false;
        }

        // 9. PINDAHKAN DATA BARANG BARU
        if ($all_ok) {
            $sql_barang = "
            SELECT * FROM history_n_temp_barang_pemutihan
            WHERE id_ba = $id_ba
              AND pending_status = 1
              AND status = 1
            ORDER BY id ASC
        ";
            ...
            if ($hasil_barang && $hasil_barang->num_rows > 0) {
                ...
                while ($data_barang = $hasil_barang->fetch_assoc()) {
                    ...
                    $insert = "
                    INSERT INTO barang_pemutihan (
                        id_ba, id_pt, pt, po, coa,
                        kode_assets, merk, sn, user,
                        harga_beli, tahun_perolehan,
                        alasan_penghapusan, kondisi
                    ) VALUES (
                        {$escaped_barang['id_ba']},
                        {$escaped_barang['id_pt']},
                        {$escaped_barang['pt']},
                        ...
                        {$escaped_barang['harga_beli']},
                        {$escaped_barang['tahun_perolehan']},
                        {$escaped_barang['alasan_penghapusan']},
                        {$escaped_barang['kondisi']}
                    )
                ";
                ...
                if ($all_ok) {
                    $del_history_barang_baru = "
                    DELETE FROM history_n_temp_barang_pemutihan
                    WHERE id_ba = $id_ba
                      AND pending_status = 1
                      AND status = 1
                ";
                    if (!$koneksi->query($del_history_barang_baru)) $all_ok = false;
                }
            } else {
                $all_ok = false;
            }
