...
<head>
  ...
  <title>Detail BA Kerusakan</title>
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary overflow-x-hidden">
    ...
    $sql = "SELECT * FROM berita_acara_kerusakan WHERE id = $id";
    ...
    <section class="custom-table-wrapper table-wrapper bg-white position-relative">
        ...
        <div class="custom-container-btn-h2 d-flex align-items-center justify-content-center position-relative">
            <div class="custom-btn-ctr position-absolute" style="left: 0;">
                <a class='custom-btn btn btn-primary' href='surat_output_kerusakan.php?id=<?= $data['id'] ?>' target='_blank'><i class="bi bi-file-earmark-text-fill"></i></a>
                <?php
                    $sessionUser = isset($_SESSION['nama']) ? $_SESSION['nama'] : '';

                    $namaAprv1      = isset($data['pembuat']) ? trim($data['pembuat']) : '';
                    $namaAprv2      = isset($data['penyetujui']) ? trim($data['penyetujui']) : '';
                    $peminjam       = isset($data['peminjam']) ? trim($data['peminjam']) : '';
                    $atasanPeminjam = isset($data['atasan_peminjam']) ? trim($data['atasan_peminjam']) : '';
                    $diketahui      = isset($data['diketahui']) ? trim($data['diketahui']) : '';
                    ...
                    $approval5      = isset($data['approval_5']) ? intval($data['approval_5']) : 0;
                    ...
                    if ($sessionUser !== '') {
                        if ($sessionUser === $namaAprv1 && $approval1 === 0) {
                            ...
                            $tampilTombol = true;
                        } elseif ($sessionUser === $peminjam && $approval3 === 0) {
                            $tampilTombol = true;
                        } elseif ($sessionUser === $atasanPeminjam && $approval4 === 0) {
                            $tampilTombol = true;
                        } elseif ($sessionUser === $diketahui && $approval5 === 0) {
                            $tampilTombol = true;
                        }
                    }
                    ...
                    if ($tampilTombol) {
                        ...
                        if ($sessionUser === $namaAprv1) {
                            $approvalField = 'approval_1';
                        } elseif ($sessionUser === $namaAprv2) {
                            $approvalField = 'approval_2';
                        } elseif ($sessionUser === $peminjam) {
                            $approvalField = 'approval_3';
                        } elseif ($sessionUser === $atasanPeminjam) {
                            $approvalField = 'approval_4';
                        }  elseif ($sessionUser === $diketahui) {
            ...
            <h2 class="custom-h2">Detail Data Kerusakan Nomor <?= htmlspecialchars($data['nomor_ba']) ?> Periode <?php echo formatTanggalRomawi($data['tanggal']); ?></h2>
        </div>
        ...
        <table class="table table-approval" style="min-width: 100px; width: 600px;">
            ...
            <tbody>
                        <tr>
                            ...
                            <?php if ($diketahui !== "-"): ?>
                                <td><?php echo $peran['jabatan_aprv5'] . " " . $peran['departemen_aprv5'] ?></td>
                            <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <?php if ($diketahui !== "-"): ?>
                            <td>
                                <span class="border fw-bold <?= $data['approval_5'] == 1 ? 'bg-success-subtle border-success-subtle text-success' : 'bg-warning-subtle border-warning-subtle text-warning' ?>" style="border-radius: 6px; padding: 6px 12px;">
                                    <?= htmlspecialchars($data['approval_5'] == 1 ? 'Disetujui' : 'Menunggu') ?>
                                </span>
                            </td>
                            <?php endif; ?>
        ...
        <table class="table-custom table table-bordered table-striped">
            <tbody>
                <!-- <tr>
                    <th>Status: Pembuat/Penyetujui</th>
                    <td>
                        <span class="border fw-bold <?= $data['approval_1'] == 1 ? 'bg-success-subtle border-success-subtle text-success' : 'bg-warning-subtle border-warning-subtle text-warning' ?>" style="border-radius: 6px; padding: 6px 12px;">
                            <?= htmlspecialchars($data['approval_1'] == 1 ? 'Disetujui' : 'Menunggu') ?>
                        </span> 
                ...
                <tr>
                    <th>Lokasi</th>
                    <td><?= htmlspecialchars($data['pt']) ?> <?= htmlspecialchars($data['lokasi']) ?></td>
                </tr>
                <tr>
                    <th>Pengguna</th>
                ...
                <tr>
                    ...
                    <td><?= nl2br(htmlspecialchars($data['penyebab_kerusakan'])) ?></td>
                </tr>
                <tr>
                    <th>Rekomendasi MIS</th>
                    <td><?= nl2br(htmlspecialchars($data['rekomendasi_mis'])) ?></td>
                </tr>
                <!-- <tr>
                    <th>Atasan Peminjam</th>
                    <td><?= htmlspecialchars($data['atasan_peminjam']) ?></td>
                </tr> -->
                <tr>
                    <th style="vertical-align: top;">Gambar Kerusakan</th>
                    <td>
                        <?php if (count($gambarList) > 0): ?>
                            <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                                <?php foreach ($gambarList as $gambar): ?>
                                    <div>
                                        <img src="<?= htmlspecialchars($gambar) ?>" alt="Gambar Kerusakan" style="max-width: 400px; height: auto; display: block;">
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
...
$queryHist = $koneksi->prepare("
    SELECT created_at, pending_status, alasan_edit, alasan_tolak, tanggal, nomor_ba,
        jenis_perangkat, merek, no_po, sn, tahun_perolehan, kategori_kerusakan_id,
