...
$title_ba = $is_penambahan ? 'BA PENAMBAHAN' : 'BA KERUSAKAN';
...
<head>
    ...
    <title><?php echo $title_ba; ?>
<?php
        if ($data['pt'] === 'PT.MSAL (HO)') {
        ?>
            <?php echo $data['nomor_ba']; ?>/MIS-HO/<?php echo formatTanggalRomawi($data['tanggal']); ?>/<?php echo getTahun($data['tanggal']); ?>
        <?php
        } else {
        ?>
            <?php echo $data['nomor_ba']; ?>/MIS/BAK/
            ...
            RO
            <?php } ?>
            /<?php echo formatTanggalRomawi($data['tanggal']); ?>/<?php echo getTahun($data['tanggal']); ?>
        <?php
        }
        ?>
</title>
...
<body>
    ...
    <div class="content">
        ...
        if ($data['pt'] === 'PT.MSAL (HO)') {
            ...
            $posisi1 = isset($data['jabatan_pembuat']) ? trim($data['jabatan_pembuat']) : '';

            // Ambil jabatan dan departemen dari tabel data_karyawan
            $nama_pengguna2 = $data['penyetujui'];
            $posisi2 = isset($data['jabatan_penyetujui']) ? trim($data['jabatan_penyetujui']) : '';
            ...
            $posisi4 = isset($data['jabatan_atasan_peminjam']) ? trim($data['jabatan_atasan_peminjam']) : '';

            $nama_pengguna5 = $data['diketahui'];
            $posisi5 = isset($data['jabatan_diketahui']) ? trim($data['jabatan_diketahui']) : '';
            ...
            // Ambil jabatan dan departemen dari tabel data_karyawan
            $nama_pengguna3 = $data['peminjam'];
            $posisi3 = isset($data['jabatan_peminjam']) ? trim($data['jabatan_peminjam']) : '';
        ...
        <div class="ttd">
            ...
            <table style="width: 100%; text-align: center;">
                ...
                <tr>
                    <?php
                    if ($data['pt'] === 'PT.MSAL (HO)') {
                        // Normalisasi cek aktor (antisipasi '-', '', null, spasi)
                        $has_atasan = isset($data['atasan_peminjam']) && trim($data['atasan_peminjam']) !== '' && trim($data['atasan_peminjam']) !== '-';
                        $has_diketahui = isset($data['diketahui']) && trim($data['diketahui']) !== '' && trim($data['diketahui']) !== '-';

                        // Penanda supaya header "DIKETAHUI," tidak dobel
                        $header_diketahui_sudah_tampil = false;

                        foreach ($signatures as $sig) {
                            // Jika ini kolom ATASAN (HO), header-nya ditampilkan sebagai "DIKETAHUI,"
                            if ($sig['label'] === 'ATASAN PENGGUNA,') {
                                if ($has_atasan && $has_diketahui) {
                                    // Atasan + Diketahui ada -> merge header 2 kolom
                                    echo '<td colspan="2"><p>DIKETAHUI,</p></td>';
                                    $header_diketahui_sudah_tampil = true;
                                } elseif ($has_atasan) {
                                    // Hanya Atasan ada -> tetap tampil 1 kolom "DIKETAHUI,"
                            ...
                            // Jika ini aktor DIKETAHUI OLEH (HO)
                            if ($sig['label'] === 'DIKETAHUI OLEH,') {
                                // Kalau belum ditampilkan dari kolom ATASAN, tampilkan di sini
                                if (!$header_diketahui_sudah_tampil && $has_diketahui) {
                                    echo '<td><p>DIKETAHUI,</p></td>';
                                    $header_diketahui_sudah_tampil = true;
                                }
                                continue;
                ...
                <tr class="signature-row">
                    ...
                    <?php foreach ($signatures as $index => $sig): ?>
                        <?php
                        // Khusus placeholder peminjam HO (label & nama memang dikosongkan)
                        $is_peminjam_placeholder_ho = (
                            $data['pt'] === 'PT.MSAL (HO)' &&
                            isset($sig['label']) && isset($sig['nama']) &&
                            trim($sig['label']) === '' &&
                            trim($sig['nama']) === ''
                        );
                        ?>

                        <?php if ($is_peminjam_placeholder_ho): ?>
                            <td class="signature" style="width: 10%;">
                        <?php else: ?>
                        ...
                        <?php endif; ?>
                            ...
                            <div style="height: 110px; display:flex; flex-direction: column; align-items:center; justify-content:center;">
                                    ...
                                    <div style="display:flex; align-items:center; justify-content:center; gap:8px;position: relative;">
                                        ...
                                        <?php if (!empty($sig['autograph'])): ?>
                                            ...
                                            <div style="display:flex; flex-direction: column;">
                                                ...
                                                <img src="<?= $src; ?>" class="signature-img" alt="Tanda Tangan">
                                ...
                                <?php elseif ($is_peminjam_placeholder_ho): ?>
                                    ...
                                    <?php if (!empty($sig['autograph'])): ?>
                                        ...
                                        $src = "data:image/png;base64," . $imgBase64;
                                        ?>
                                        <img src="<?= $src; ?>" style="width:25mm" alt="Tanda Tangan" style="position:absolute;">
