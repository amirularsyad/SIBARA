...
elseif ($pt === 'PT.MSAL (SITE)') {
    $query              = $koneksi->query("SELECT nama FROM data_karyawan_test WHERE posisi = 'KTU' AND pt = 'PT.MSAL (SITE)' LIMIT 1");
    $data               = $query->fetch_assoc();
    $penyetujui         = $data ? $data['nama'] : '-';

    $query              = $koneksi->query("SELECT nama FROM data_karyawan_test WHERE posisi = 'GM' AND pt = 'PT.MSAL (SITE)' LIMIT 1");
    $data               = $query->fetch_assoc();
    $pembuat            = $data ? $data['nama'] : '-';
}
...
elseif ($pt !== '') {
    ...
    $query              = $koneksi->query("SELECT nama FROM data_karyawan_test WHERE posisi = 'GM' AND pt = $pt LIMIT 1");
    $data               = $query->fetch_assoc();
    $pembuat            = $data ? $data['nama'] : '-';
}
...
$old_stmt = $koneksi->prepare("SELECT 
    ...
    pt, id_pt, lokasi, nama_pembuat, pembuat, penyetujui, peminjam, atasan_peminjam, diketahui,
...
if (!empty($perubahan) && $ada_approval) {
    ...
    $insert_old = $koneksi->prepare("
        INSERT INTO history_n_temp_ba_kerusakan (
            ...
            nama_pembuat, pembuat, penyetujui, peminjam, atasan_peminjam, diketahui,
            approval_1, approval_2, approval_3, approval_4, approval_5,
            autograph_1, autograph_2, autograph_3, autograph_4, autograph_5,
            tanggal_approve_1, tanggal_approve_2, tanggal_approve_3, tanggal_approve_4, tanggal_approve_5,
            file_created
    ...
    $insert_old->bind_param(
        "issssssssssssssssisssssssiiiiisssssssssss",
        $old_data_array['id'],
        $pending_approver,
        $alasan_perubahan,
...
if (!empty($perubahan) && $semua_approval_nol) {
    ...
    ");
    $insert_old->bind_param(
        "isssssssssssssississsssssiiiiisssssssssss",
        $old_data_array['id'],
        $pending_approver,
        $alasan_perubahan,
        $old_data_array['tanggal'],
    ...
    $update_real->bind_param(
        "ssssssssssisssisssssssiiiiissssssssss" . "i",
        $new_data_array['tanggal'],
        $new_data_array['nomor_ba'],
        $new_data_array['jenis_perangkat'],
        $new_data_array['merek'],
